...
# Drop redundant columns and rename 'group'.
stopifnot(identical(new_colData[["group"]], new_colData[["Group"]]))
new_colData[["treatment"]] <- factor(new_colData[["group"]])
...
new_colData[["sex"]] <- factor(ifelse(new_colData[["sex"]] == "1", "M", "F"))
...
# Needed for DE analysis
new_colData[["Underweight24"]] <- dplyr::case_when(
  is.na(new_colData[["WAZ24"]]) ~ NA,
  new_colData[["WAZ24"]] < -2 ~ TRUE,
  new_colData[["WAZ24"]] > -2 & new_colData[["WAZ24"]] < 2 ~ FALSE,
  new_colData[["WAZ24"]] > 2 ~ FALSE)
new_colData[["Anaemic0"]] <- dplyr::case_when(
  is.na(new_colData[["hb0"]]) ~ NA,
  new_colData[["hb0"]] < 11 ~ TRUE,
  new_colData[["hb0"]] >= 11 ~ FALSE)
new_colData[["Anaemic24"]] <- dplyr::case_when(
  is.na(new_colData[["hb24"]]) ~ NA,
  new_colData[["hb24"]] < 11 ~ TRUE,
  new_colData[["hb24"]] >= 11 ~ FALSE)
...
as.data.frame(colData(sce)) %>%
  ...
  janitor::tabyl(treatment, timepoint) %>%
  knitr::kable(caption = "Number of samples per treatment group/timepoint.")
```

### Unblinding of treatment groups

Following publication of the [main paper describing this study](https://pubmed.ncbi.nlm.nih.gov/34496174/), we obtained access to the unblinded treatment groups.
...
```{r}
sce$treatment_unblinded <- dplyr::case_when(
  sce$treatment == "A" ~ "10 mg zinc MNP",
  sce$treatment == "B" ~ "10 mg zinc tablet",
  sce$treatment == "C" ~ "placebo powder")
```
...
Ultimately, the 'LC475 mini bulk back up 1:10 dilution' (`C084_LC475_mini_bulk_back_up_1:10`; henceforth abbreviate to `LC475`) and 'LC476 mini bulk original (1ng/uL)' (`LCE_476_BU_8_cycle`; henceforth abbreviate to `LC476`) library preparations were taken forward.
`LC475` and `LC476` were sequenced in run `NN183` and `LC476` was re-sequenced in run `NN195`.
...
**It was ultimately decided to re-sequence `LC476` to obtain additional data.**

<aside>
The analysis below is what led to this decision, but now includes the re-sequencing data.
</aside>
...
Figure \@ref(fig:saturation) shows that the hemoglobin subunit genes are sequenced to near-total saturation.
The more deeply sequenced samples on plate `LC476` have correspondingly higher average sequencing saturation of non-hemoglobin subunit genes (median = `r percent(median(saturation_no_hemo[sce$plate_number == "LC476"]))`) than for samples on the `LC475` plate (median  = `r percent(median(saturation_no_hemo[sce$plate_number == "LC475"]))`).
...
**As already mentioned, it was ultimately decided to re-sequence `LC476`.**
...
ii <- names(sort(cor_i)[c(1:3, 17358:17359, 34715:34717)])
for (i in ii) {
  g <- i
  plot(
    x = X[i, ],
