...
```{r}
design_lc476_with_sex <- model.matrix(~0 + group + sex_lc476, x_lc476$samples)
...
```{r}
tmp <- unique(x_lc476$samples[, c("id", "treatment", "age0")])
tmp$age0_cut <- cut(tmp$age0, breaks = c(8.5, 10, 11.5))
```
...
```{r}
tmp <- unique(x_lc476$samples[, c("id", "treatment", "HAZ24", "Stunted24")])
```
...
```{r}
tmp <- unique(x_lc476$samples[, c("id", "treatment", "WHZ24", "Wasted24")])
```
...
```{r}
tmp <- unique(x_lc476$samples[, c("id", "treatment", "WAZ24", "Underweight24")])
```
...
```{r}
tmp <- unique(x_lc476$samples[, c("id", "treatment", "hb24", "Anaemic24")])
```
