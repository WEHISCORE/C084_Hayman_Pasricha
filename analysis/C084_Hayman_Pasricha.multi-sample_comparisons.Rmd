...
# TODO: Check sex coding and move this into preprocessing
sce$sex <- ifelse(sce$sex == "1", "M", "F")
names(sex_colours) <- c("F", "M")
```
...
```{r}
# TODO: Play with arguments of filterByExpr()
...
In this study, it is of interest to see which genes are expressed at different levels between the different treatments and timepoints, as well as additional covariates measures on the study subjects.
...
To get started, a design matrix is set up with both the timepoint, treatment, subject, and plate number (batch) information.
...
Contrasts for pairwise comparisons between treatment groups and timepoints, within each sample and blocking on plate number, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.
...
Where experimental or technical effects can be modelled using a random effect, another possibility in `r BiocStyle::Biocpkg("limma")` is to estimate correlations using `duplicateCorrelation()` by specifying a block argument for both this function and in the `lmFit()` linear modelling step.
