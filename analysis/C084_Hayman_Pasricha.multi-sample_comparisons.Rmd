...
### Differences in delta between groups
...
createDEGOutputs(
  ...
  efit = efit_,
  x = x_,
  prefix = "both_plates.delta_comparisons",
  suffix = NULL,
  groups = "treatment.timepoint",
  sample.cols = "treatment_colours",
  annotation_col = c("plate_number", "treatment", "timepoint", "sex"))
...
### PDF version of MA plots

Tom asked for a PDF version of the MA plots; see [`output/paper_figures/both_plates.Figure2.MA-plots.pdf`](../output/paper_figures/both_plates.Figure2.MA-plots.pdf).

```{r}
dir.create(here("output", "paper_figures"))
pdf(
  here("output", "paper_figures", "both_plates.Figure2.MA-plots.pdf"),
  height = 6,
  width = 6)
for (cn in colnames(efit)) {
  plotMA(
    efit,
    coef = cn,
    xlim = c(4, 12),
    ylim = c(-1.3, 1.3),
    status = decideTests(efit)[, cn],
    main = cn)
}
dev.off()
```
...
Tom asked for a PDF version of the MA plots; see [`output/paper_figures/both_plates.Figure3.MA-plots.pdf`](../output/paper_figures/both_plates.Figure3.MA-plots.pdf).

```{r}
pdf(
  here("output", "paper_figures", "both_plates.Figure3.MA-plots.pdf"),
  height = 6,
  width = 6)
plotMA(
  efit_stunted24,
  ...
  xlim = c(4, 12),
  ylim = c(-1.3, 1.3),
  status = decideTests(efit_stunted24),
  main = "Stunted24")
plotMA(
  efit_wasted24,
  coef = "Wasted24",
  xlim = c(4, 12),
  ylim = c(-1.3, 1.3),
  status = decideTests(efit_wasted24),
  main = "Wasted24")
plotMA(
  efit_underweight24,
  coef = "Underweight24",
  xlim = c(4, 12),
  ylim = c(-1.3, 1.3),
  status = decideTests(efit_underweight24),
  main = "Underweight24")
dev.off()
```
...
Please see [`output/DEGs/LC476/`](../output/DEGs/LC476/) for spreadsheets of these DEG lists, including results of GO and KEGG analyses using the `goana()` and `kegga()` functions from the `r BiocStyle::Biocpkg("limma")` package, as well as heatmaps and Venn diagrams of the DEG lists and interactive `r BiocStyle::Biocpkg("Glimma")` plots of the differential expression results.
The heatmaps and right panels of the `r BiocStyle::Biocpkg("Glimma")` plots use the batch-corrected log-CPMs.
Group labels may be abbreviated in the `r BiocStyle::Biocpkg("Glimma")` plots.
For example, when looking at the effect of `Stunted24`, the group `A.F` denotes samples with treatment `A` (**A**) that were not stunted (**F**ALSE); group `B.T` denotes samples with treatment `B` (**B**) that were stunted (**T**RUE); etc.

```{r}
deg_dir <- here("output", "DEGs", "LC476")
dir.create(deg_dir, recursive = TRUE)
```
...
# Create outputs
x_ <- x_lc476[, x_lc476$sample$timepoint == "endline"]
o <- order(
  x_$samples$treatment,
  x_$samples$sex,
  x_$samples$id)
...
createDEGOutputs(
  ...
  efit = efit_,
  x = x_,
  prefix = "LC476.endline_treatment_comparisons",
  suffix = NULL,
  groups = "treatment",
  sample.cols = "treatment_colours",
  annotation_col = c("treatment", "sex"))
```
...
```{r}
design_lc476_with_sex <- model.matrix(~0 + group + sex, x_lc476$samples)
colnames(design_lc476_with_sex) <- sub(
  "sex|group", 
  "",
  colnames(design_lc476_with_sex))
contr_matrix_lc476_with_sex <- makeContrasts(
  A.endline_vs_A.baseline = A.endline - A.baseline,
  B.endline_vs_B.baseline = B.endline - B.baseline,
  C.endline_vs_C.baseline = C.endline - C.baseline,
  A.endline_vs_B.endline = A.endline - B.endline,
  ...
  A_delta_vs_C_delta = (A.endline - A.baseline) - (C.endline - C.baseline),
  B_delta_vs_C_delta = (B.endline - B.baseline) - (C.endline - C.baseline),
  levels = colnames(design_lc476_with_sex))
vfit_lc476_with_sex <- voomLmFit(
  x_lc476,
  design_lc476_with_sex,
  x_lc476$samples$id,
  plot = FALSE, 
  normalize.method = "quantile")
vfit_lc476_with_sex <- contrasts.fit(
  vfit_lc476_with_sex,
  contrasts = contr_matrix_lc476_with_sex)
efit_with_sex <- eBayes(vfit_lc476_with_sex, robust = TRUE)
```
...
createDEGOutputs(
  ...
  efit = efit_,
  x = x_,
  prefix = "LC476.timepoint_comparisons",
  suffix = NULL,
  groups = "treatment.timepoint",
  sample.cols = "treatment_colours",
  annotation_col = c("treatment", "timepoint", "sex"))
```
...
createDEGOutputs(
  ...
  efit = efit_,
  x = x_,
  prefix = "LC476.delta_comparisons",
  suffix = NULL,
  groups = "treatment.timepoint",
  sample.cols = "treatment_colours",
  annotation_col = c("treatment", "timepoint", "sex"))
...
Tom asked for a PDF version of the MA plots; see [`output/paper_figures/LC476.Figure2.MA-plots.pdf`](../output/paper_figures/LC476.Figure2.MA-plots.pdf).

```{r}
dir.create(here("output", "paper_figures"))
pdf(
  here("output", "paper_figures", "LC476.Figure2.MA-plots.pdf"),
  height = 6,
  width = 6)
for (cn in colnames(efit_lc476)) {
  plotMA(
    efit_lc476,
    coef = cn,
    xlim = c(4, 12),
    ylim = c(-1.3, 1.3),
    status = decideTests(efit_lc476)[, cn],
    main = cn)
}
dev.off()
```
...
Contrasts for association of gene expression with `HAZ24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$HAZ24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + HAZ24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
createDEGOutputs(
  ...
  groups = "groups",
  sample.cols = "Stunted24",
  annotation_col = c("treatment", "HAZ24", "Stunted24"))
```
...
Contrasts for association of gene expression with `Stunted24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$Stunted24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + Stunted24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
Contrasts for association of gene expression with `WHZ24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$WHZ24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + WHZ24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
createDEGOutputs(
  ...
  groups = "groups",
  sample.cols = "Wasted24",
  annotation_col = c("treatment", "WHZ24", "Wasted24"))
```
...
Contrasts for association of gene expression with `Wasted24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$Wasted24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + Wasted24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
Contrasts for association of gene expression with `WAZ24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$WAZ24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + WAZ24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
createDEGOutputs(
  ...
  groups = "groups",
  sample.cols = "Underweight24",
  annotation_col = c("treatment", "WAZ24", "Underweight24"))
```
...
Contrasts for association of gene expression with `Underweight24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$Underweight24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + Underweight24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
Tom asked for a PDF version of the MA plots; see [`output/paper_figures/LC476.Figure3.MA-plots.pdf`](../output/paper_figures/LC476.Figure3.MA-plots.pdf).

```{r}
pdf(
  here("output", "paper_figures", "LC476.Figure3.MA-plots.pdf"),
  height = 6,
  width = 6)
plotMA(
  efit_stunted24,
...
Contrasts for association of gene expression with `hb24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$hb24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + hb24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
...
createDEGOutputs(
  ...
  groups = "groups",
  sample.cols = "Anaemic24",
  annotation_col = c("treatment", "hb24", "Anaemic24"))
```
...
Contrasts for association of gene expression with `Anaemic24`, accounting for `group`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.

```{r, results = "hide"}
x_ <- x_lc476[, !is.na(x_lc476$samples$Anaemic24) & x_lc476$samples$timepoint == "endline"]

design_ <- model.matrix(~0 + group + Anaemic24, x_$samples)
colnames(design_) <- sub("group", "", colnames(design_))
