...
Figure \@ref(fig:mds) shows that samples can be seen to cluster strongly by experimental batch over dimension 1.
...
par(mfrow = c(3, 2))
plotMDS(lcpm, pch = 16, col = plate_number_colours[x$samples$plate_number])
legend(
  "bottom",
...
plotMDS(
  lcpm,
  pch = 16,
  col = plate_number_colours[x$samples$plate_number],
  dim = c(3, 4))

plotMDS(lcpm, pch = 16, col = treatment_colours[x$samples$treatment])
legend(
  "bottom",
...
plotMDS(
  lcpm,
  pch = 16,
  col = treatment_colours[x$samples$treatment],
  dim = c(3, 4))

plotMDS(lcpm, pch = 16, col = timepoint_colours[x$samples$timepoint])
legend(
  "bottom",
...
plotMDS(
  lcpm,
  pch = 16,
  col = timepoint_colours[x$samples$timepoint],
  dim = c(3, 4))
```

Figures \@ref(fig:mds-lc475) and \@ref(fig:mds-lc476) show similar plots but when restricted to batch `LC475` or `LC476`, respectively.
In each case, we observe some variation along dimension 1 by timepoint but little association with treatment over dimensions $1-4$.
...
x_ <- x[, x$samples$plate_number == "LC475"]
lcpm <- cpm(x_, log = TRUE)

par(mfrow = c(2, 2))
plotMDS(lcpm, pch = 16, col = treatment_colours[x_$samples$treatment])
legend(
  "bottom",
  col = treatment_colours,
  legend = levels(x_$samples$treatment),
  pch = 16,
  title = "treatment")
plotMDS(
  lcpm,
  pch = 16,
  col = treatment_colours[x_$samples$treatment],
  dim = c(3, 4))

plotMDS(lcpm, pch = 16, col = timepoint_colours[x_$samples$timepoint])
legend(
  "bottom",
  col = timepoint_colours,
  legend = levels(x_$samples$timepoint),
  pch = 16,
  title = "timepoint")
plotMDS(
  lcpm,
  pch = 16,
  col = timepoint_colours[x_$samples$timepoint],
  dim = c(3, 4))
```

```{r mds-lc476, fig.cap = "MDS plots of `LC476` log-CPM values over dimensions 1 and 2 and dimensioned 3 and 4 with samples coloured by treatment and timepoint. Distances on the plot correspond to the leading fold-change, which is the average (root-mean-square) log2-fold-change for the 500 genes most divergent between each pair of samples by default.", fig.asp = 1}
x_ <- x[, x$samples$plate_number == "LC476"]
lcpm <- cpm(x_, log = TRUE)
...
This suggests we may observe differential expression between timepoints but not much between treatments.
...
To get started, a design matrix is set up with the `group` (i.e. the combination of `timepoint` and `treatment`) and `plate_number` (i.e. batch) information.
...
contr.matrix <- makeContrasts(
  ...
  Adelta_vs_B_delta = (A.endline - A.baseline) - (B.endline - B.baseline),
  Adelta_vs_C_delta = (A.endline - A.baseline) - (C.endline - C.baseline),
  Bdelta_vs_C_delta = (B.endline - B.baseline) - (C.endline - C.baseline),
...
createDEGOutputs(
  ...
  o = order(
    ...
    x_$samples$id),
  groups = paste(
    sub("LC47", "", x_$samples$plate_number),
    x_$samples$treatment,
    sep = "."),
  sample.cols = x_$samples$treatment_colours)
```
...
```{r}
design <- model.matrix(~0 + group + sex + plate_number, x$samples)
colnames(design) <- sub("plate\\_number|sex|group", "", colnames(design))
vfit_with_sex <- voomLmFit(x, design, x$samples$id, plot = FALSE)
contr.matrix_with_sex <- makeContrasts(
  A.endline_vs_A.baseline = A.endline - A.baseline,
  B.endline_vs_B.baseline = B.endline - B.baseline,
  C.endline_vs_C.baseline = C.endline - C.baseline,
  A.endline_vs_B.endline = A.endline - B.endline,
  ...
  Bdelta_vs_C_delta = (B.endline - B.baseline) - (C.endline - C.baseline),
  levels = colnames(design))
vfit_with_sex <- contrasts.fit(vfit_with_sex, contrasts = contr.matrix_with_sex)
efit_with_sex <- eBayes(vfit_with_sex)
```
...
createDEGOutputs(
  ...
  suffix = NULL,
  o = order(
    x$sample$plate_number,
    x$sample$treatment,
    x$sample$timepoint,
    x$sample$sex,
    x$sample$id),
  groups = paste(
    sub("LC47", "", x$sample$plate_number),
    x$sample$treatment,
    ifelse(x$sample$timepoint == "baseline", "b", "e"),
    sep = "."),
  sample.cols = x$sample$timepoint_colours)
```

**TODO** Some parallel co-ordinates plots of the genes that change?

```{r}
ggplot(
  data = data.frame(
    x = x$samples$timepoint,
    y = cpm(x, log = TRUE)["FTH1", ],
    id = x$samples$id,
    treatment = x$samples$treatment,
    timepoint = x$samples$timepoint,
    plate_number = x$samples$plate_number),
  aes(x = x, y = y, col = treatment)) +
  geom_point() +
  geom_line(aes(group = id)) +
  facet_grid(plate_number ~ treatment) +
...
createDEGOutputs(
  ...
  groups = paste(
    ...
    sep = "."),
  sample.cols = x_$sample$Stunted24)
```
...
createDEGOutputs(
  ...
  groups = paste(
    ...
    sep = "."),
  sample.cols = x_$sample$Wasted24)
```
...
createDEGOutputs(
  ...
  groups = paste(
    ...
    sep = "."),
  sample.cols = x_$sample$Underweight24)
```
...
design_ <- model.matrix(
  ~0 + group + Underweight24 + plate_number,
  x_$samples)
colnames(design_) <- sub("plate\\_number|group", "", colnames(design_))
...
Contrasts for association of gene expression with `hb24`, accounting for `group` `plate_number`, are set up in `r BiocStyle::Biocpkg("limma")` using the `makeContrasts()` function.
...
createDEGOutputs(
  ...
  groups = paste(
    ...
    sep = "."),
  sample.cols = x_$sample$Anaemic24)
```
