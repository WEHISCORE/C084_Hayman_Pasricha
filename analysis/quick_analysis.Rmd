...
```{r}
y <- DGEList(counts(sce), samples = colData(sce), genes = rowData(sce))
...
y$samples$abbreviated_g <- paste0(
  dplyr::case_when(
    y$samples$regimen == "placebo" ~ "plcb",
    y$samples$regimen == "zinc_10mg_tab" ~ "Zn_10mg",
    y$samples$regimen == "high_zinc_low_iron_mnp" ~ "Zn_Fe_MNP"),
  ".", 
  dplyr::case_when(
    y$samples$timepoint == "baseline" ~ "base",
    y$samples$timepoint == "endline" ~ "end"))
...
my_contrasts <- makeContrasts(
  ...
  endline.zinc_10mg_tab_vs_placebo = zinc_10mg_tab.endline - placebo.endline,
  endline.high_zinc_low_iron_mnp_vs_placebo = high_zinc_low_iron_mnp.endline -
    placebo.endline,
  levels = design)
```
...
glMDPlot(
  ...
  counts = fit$counts,
  anno = fit$genes,
  groups = fit$samples$abbreviated_g,
  transform = TRUE,
  main = "placebo.endline_vs_baseline",
  status = decideTests(qlf_placebo),
  side.ylab = "logCPM",
  path = here("analysis"),
  html = "placebo.endline_vs_baseline.MD_plot",
  launch = FALSE)
```

`r glMDRmd("placebo.endline_vs_baseline.MD_plot")`
...
glMDPlot(
  ...
  groups = fit$samples$abbreviated_g,
  transform = TRUE,
  main = "zinc_10mg_tab.endline_vs_baseline",
  status = decideTests(qlf_zinc_10mg_tab),
  side.ylab = "logCPM",
  path = here("analysis"),
  html = "zinc_10mg_tab.endline_vs_baseline.MD_plot",
  launch = FALSE)
```

`r glMDRmd("zinc_10mg_tab.endline_vs_baseline.MD_plot")`
...
```{r}
glMDPlot(
  qlf_high_zinc_low_iron_mnp,
  counts = fit$counts,
  anno = fit$genes,
  groups = fit$samples$abbreviated_g,
  transform = TRUE,
  main = "high_zinc_low_iron_mnp.endline_vs_baseline",
  status = decideTests(qlf_high_zinc_low_iron_mnp),
  side.ylab = "logCPM",
  path = here("analysis"),
  html = "high_zinc_low_iron_mnp.endline_vs_baseline.MD_plot",
  launch = FALSE)
```

`r glMDRmd("high_zinc_low_iron_mnp.endline_vs_baseline.MD_plot")`

## endline.zinc_10mg_tab_vs_placebo

```{r}
qlf_endline.zinc_10mg_tab_vs_placebo <- glmQLFTest(fit, contrast = my_contrasts[, "endline.zinc_10mg_tab_vs_placebo"])
summary(decideTests(qlf_endline.zinc_10mg_tab_vs_placebo))
rmarkdown::paged_table(as.data.frame(topTags(qlf_endline.zinc_10mg_tab_vs_placebo, n = Inf, p.value = 0.05)))
```

```{r}
tt_endline.zinc_10mg_tab_vs_placebo <- topTags(qlf_endline.zinc_10mg_tab_vs_placebo, n = Inf, p.value = 0.05)
pdf(here("output", "figures", "endline.zinc_10mg_tab_vs_placebo.pdf"))
for (i in seq_len(nrow(tt_endline.zinc_10mg_tab_vs_placebo))) {
  message(i)
  g <- rownames(tt_endline.zinc_10mg_tab_vs_placebo)[i]
  p <- plotExpression(
    sce,
    g,
    x = I(paste0(sce$regimen, ".", sce$timepoint)), colour_by = "timepoint",
    ...
    ggtitle(
      paste0(
        g,
        " (FDR = ", 
        signif(tt_endline.zinc_10mg_tab_vs_placebo[g, "FDR"][[1]], 1),
        ")")) +
    stat_summary(
      fun.data = "mean_cl_boot",
      geom = "crossbar",
...
```{r}
glMDPlot(
  qlf_endline.zinc_10mg_tab_vs_placebo,
  counts = fit$counts,
  anno = fit$genes,
  groups = fit$samples$abbreviated_g,
  transform = TRUE,
  main = "endline.zinc_10mg_tab_vs_placebo",
  status = decideTests(qlf_endline.zinc_10mg_tab_vs_placebo),
  side.ylab = "logCPM",
  path = here("analysis"),
  html = "endline.zinc_10mg_tab_vs_placebo.MD_plot",
  launch = FALSE)
```

`r glMDRmd("endline.zinc_10mg_tab_vs_placebo.MD_plot")`

## endline.high_zinc_low_iron_mnp_vs_placebo

```{r}
qlf_endline.high_zinc_low_iron_mnp_vs_placebo <- glmQLFTest(fit, contrast = my_contrasts[, "endline.high_zinc_low_iron_mnp_vs_placebo"])
summary(decideTests(qlf_endline.high_zinc_low_iron_mnp_vs_placebo))
rmarkdown::paged_table(as.data.frame(topTags(qlf_endline.high_zinc_low_iron_mnp_vs_placebo, n = Inf, p.value = 0.05)))
```

```{r}
tt_endline.high_zinc_low_iron_mnp_vs_placebo <- topTags(qlf_endline.high_zinc_low_iron_mnp_vs_placebo, n = Inf, p.value = 0.05)
pdf(here("output", "figures", "endline.high_zinc_low_iron_mnp_vs_placebo.pdf"))
for (i in seq_len(nrow(tt_endline.high_zinc_low_iron_mnp_vs_placebo))) {
  message(i)
  g <- rownames(tt_endline.high_zinc_low_iron_mnp_vs_placebo)[i]
  p <- plotExpression(
    sce,
    g,
    x = I(paste0(sce$regimen, ".", sce$timepoint)), colour_by = "timepoint",
    ...
    ggtitle(
      paste0(
        g,
        " (FDR = ", 
        signif(tt_endline.high_zinc_low_iron_mnp_vs_placebo[g, "FDR"][[1]], 1),
        ")")) +
    stat_summary(
      fun.data = "mean_cl_boot",
      geom = "crossbar",
...
```{r}
glMDPlot(
  qlf_endline.high_zinc_low_iron_mnp_vs_placebo,
  counts = fit$counts,
  anno = fit$genes,
  groups = fit$samples$abbreviated_g,
  transform = TRUE,
  main = "endline.high_zinc_low_iron_mnp_vs_placebo",
  status = decideTests(qlf_endline.high_zinc_low_iron_mnp_vs_placebo),
  side.ylab = "logCPM",
  path = here("analysis"),
  html = "endline.high_zinc_low_iron_mnp_vs_placebo.MD_plot",
  launch = FALSE)
```

`r glMDRmd("endline.high_zinc_low_iron_mnp_vs_placebo.MD_plot")`
