...
group_colours <- setNames(
  RColorBrewer::brewer.pal(nlevels(sce$group), "Accent"),
  levels(sce$group))
...
```{r, fig.asp = 1}
plotHighestExprs(
  sce[, sce$group == "A" & sce$timepoint == "baseline"],
  n = 50) +
  ggtitle("A.baseline")
```
...
```{r, fig.asp = 1}
plotHighestExprs(
  sce[, sce$group == "A" & sce$timepoint == "endline"],
  n = 50) +
  ggtitle("A.endline")
```
...
```{r, fig.asp = 1}
plotHighestExprs(
  sce[, sce$group == "B" & sce$timepoint == "baseline"],
  n = 50) +
  ggtitle("B.baseline")
```
...
```{r, fig.asp = 1}
plotHighestExprs(
  sce[, sce$group == "B" & sce$timepoint == "endline"],
  n = 50) +
  ggtitle("B.endline")
```
...
```{r, fig.asp = 1}
plotHighestExprs(
  sce[, sce$group == "C" & sce$timepoint == "baseline"],
  n = 50) +
  ggtitle("C.baseline")
```
...
```{r, fig.asp = 1}
plotHighestExprs(
  sce[, sce$group == "C" & sce$timepoint == "endline"],
  n = 50) +
  ggtitle("C.endline")
```
...
plotHighestExprs(
  sce[!grepl("^MT|^RPS|^RPL", rownames(sce)),
      sce$group == "A" & sce$timepoint == "baseline"],
  n = 50) +
  ggtitle("A.baseline: Excluding mitochondrial and ribosomal protein genes")
```

```{r, fig.asp = 1}
plotHighestExprs(
  sce[!grepl("^MT|^RPS|^RPL", rownames(sce)),
      sce$group == "A" & sce$timepoint == "endline"],
  n = 50) +
  ggtitle("A.endline: Excluding mitochondrial and ribosomal protein genes")
```

```{r, fig.asp = 1}
plotHighestExprs(
  sce[!grepl("^MT|^RPS|^RPL", rownames(sce)),
      sce$group == "B" & sce$timepoint == "baseline"],
  n = 50) +
  ggtitle("B.baseline: Excluding mitochondrial and ribosomal protein genes")
```

```{r, fig.asp = 1}
plotHighestExprs(
  sce[!grepl("^MT|^RPS|^RPL", rownames(sce)),
      sce$group == "B" & sce$timepoint == "endline"],
  n = 50) +
  ggtitle("B.endline: Excluding mitochondrial and ribosomal protein genes")
```

```{r, fig.asp = 1}
plotHighestExprs(
  sce[!grepl("^MT|^RPS|^RPL", rownames(sce)),
      sce$group == "C" & sce$timepoint == "baseline"],
  n = 50) +
  ggtitle("C.baseline: Excluding mitochondrial and ribosomal protein genes")
```

```{r, fig.asp = 1}
plotHighestExprs(
  sce[!grepl("^MT|^RPS|^RPL", rownames(sce)),
      sce$group == "C" & sce$timepoint == "endline"],
  n = 50) +
  ggtitle("C.endline: Excluding mitochondrial and ribosomal protein genes")
```
...
y <- y[, !discarded]
y$samples$g <- paste0(y$samples$group, ".", y$samples$timepoint)
...
my_contrasts <- makeContrasts(
  A.endline_vs_baseline = A.endline - A.baseline,
  B.endline_vs_baseline = B.endline - B.baseline,
  C.endline_vs_baseline = C.endline - C.baseline,
  levels = design)
```

## A.endline_vs_baseline

```{r}
qlf_A <- glmQLFTest(fit, contrast = my_contrasts[, "A.endline_vs_baseline"])
summary(decideTests(qlf_A))
rmarkdown::paged_table(as.data.frame(topTags(qlf_A, n = Inf, p.value = 0.05)))
```

```{r}
tt_A <- topTags(qlf_A, n = Inf, p.value = 0.05)
pdf(here("output", "figures", "C.endline_vs_baseline.pdf"))
for (i in seq_len(nrow(tt_A))) {
  message(i)
  g <- rownames(tt_A)[i]
  p <- plotExpression(
    sce,
    g,
    x = I(paste0(sce$group, ".", sce$timepoint)), colour_by = "timepoint",
...
## B.endline_vs_baseline

```{r}
qlf_B <- glmQLFTest(fit, contrast = my_contrasts[, "B.endline_vs_baseline"])
summary(decideTests(qlf_B))
rmarkdown::paged_table(as.data.frame(topTags(qlf_B, n = Inf, p.value = 0.05)))
```
...
```{r}
tt_B <- topTags(qlf_B, n = Inf, p.value = 0.05)
pdf(here("output", "figures", "B.endline_vs_baseline.pdf"))
for (i in seq_len(nrow(tt_B))) {
  message(i)
  g <- rownames(tt_B)[i]
  p <- plotExpression(
    sce,
...
## C.endline_vs_baseline

```{r}
qlf_C <- glmQLFTest(fit, contrast = my_contrasts[, "C.endline_vs_baseline"])
summary(decideTests(qlf_C))
rmarkdown::paged_table(as.data.frame(topTags(qlf_C, n = Inf, p.value = 0.05)))
```

```{r}
tt_C <- topTags(qlf_C, n = Inf, p.value = 0.05)
pdf(here("output", "figures", "C.endline_vs_baseline.pdf"))
for (i in seq_len(nrow(tt_C))) {
  message(i)
  g <- rownames(tt_C)[i]
  p <- plotExpression(
    sce,
...
```{r}
vennDiagram(cbind(decideTests(qlf_A), decideTests(qlf_B), decideTests(qlf_C)))
```

```{r}
g <- unique(c(rownames(tt_A), rownames(tt_B), rownames(tt_C)))
```
...
plotHeatmap(
  ...
  center = TRUE,
  zlim = c(-3, 3),
  colour_columns_by = c("plate_number", "group", "timepoint"),
  order_columns_by  = c("plate_number", "group", "timepoint"),
  cluster_cols = FALSE)
dev.off()
```
...
plotHeatmap(
  ...
  center = TRUE,
  zlim = c(-3, 3),
  colour_columns_by = c("group", "timepoint"),
  order_columns_by  = c("group", "timepoint"),
  cluster_cols = FALSE,
