...
**TODO**: Incorporate updated sample sheet.
...
However, this was not the original plan.
...
Likely what happened, according to Daniel Brown, is that there was carry-over of a large number of molecules without adapters, which meant that the `LCE475` library contained few sequence-able molecules.
...
- `C084_LC475_mini_bulk_back_up_1:10`: **TODO** Get description of what was done to this plate.
- `LCE_476_BU_8_cycle`: **TODO** Get description of what was done to this plate. 

These two plates are essentially technical replicates.

# Have we sequenced enough?

## Motivation

**TODO**: Spiel about whether we've sequenced enough.
...
plot(
  density(
    reads_ls[sce$plate_number == "C084_LC475_mini_bulk_back_up_1:10"]),
...
plot(
  density(
    reads_ls[sce$plate_number == "LCE_476_BU_8_cycle"]),
  xlab = expression(log[10](n[reads])),
...
```{r, fig.asp = 1 / 2}
haem_prop <- colSums(assay(sce, "read_counts")[hemo_set, ]) / 10 ^ reads_ls
par(mfrow = c(1, 2))
plot(
  density(
    haem_prop[sce$plate_number == "C084_LC475_mini_bulk_back_up_1:10"],
    from = 0,
    to = 1),
...
plot(
  density(
    haem_prop[sce$plate_number == "LCE_476_BU_8_cycle"],
    from = 0,
    to = 1),
  xlab = expression(prop[hemo]),
  main = "LCE_476_BU_8_cycle",
...
10X use a [formula to quantify the sequencing saturation](https://kb.10xgenomics.com/hc/en-us/articles/115003646912-How-is-sequencing-saturation-calculated-): 

> 1 - (n_deduped_reads / n_reads)
...
plot(
  density(
    saturation[sce$plate_number == "C084_LC475_mini_bulk_back_up_1:10"],
    from = 0,
    to = 1),
  xlab = "saturation",
  main = "C084_LC475_mini_bulk_back_up_1:10",
...
plot(
  density(
    saturation[sce$plate_number == "LCE_476_BU_8_cycle"],
    from = 0,
    to = 1),
  xlab = "saturation",
  main = "LCE_476_BU_8_cycle",
...
plot(
  density(
    saturation_no_hemo[sce$plate_number == "C084_LC475_mini_bulk_back_up_1:10"],
    from = 0,
    to = 1),
  xlab = "saturation",
  main = "C084_LC475_mini_bulk_back_up_1:10",
...
plot(
  density(
    saturation_no_hemo[sce$plate_number == "LCE_476_BU_8_cycle"],
    from = 0,
    to = 1),
  xlab = "saturation",
  main = "LCE_476_BU_8_cycle",
...
plot(
  density(
    saturation_hemo[sce$plate_number == "C084_LC475_mini_bulk_back_up_1:10"],
    from = 0,
    to = 1),
  xlab = "saturation",
  main = "C084_LC475_mini_bulk_back_up_1:10",
...
plot(
  density(
    saturation_hemo[sce$plate_number == "LCE_476_BU_8_cycle"],
    from = 0,
    to = 1),
  xlab = "saturation",
  main = "LCE_476_BU_8_cycle",
