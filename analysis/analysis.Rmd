...
knitr::kable(
  ...
  caption = "Desrcriptions of additional library preparations.")
```

Ultimately, the 'LC475 mini bulk back up 1:10 dilution' (henceforth abbreviate to `LC475`) and 'LC476 mini bulk original (1ng/uL)' (henceforth abbreviate to `LC476`) library preparations were taken forward and sequenced in run `NN183`.
...
```{r}
# Some useful colours
plate_number_colours <- c("LC475" = "dodgerBlue", "LC476" = "orange")
...
**TODO** Figure \@ref(fig: breakdown-by-plate-number-post-qc) and its predecessor.
...
**TODO**: Were the 2 `plate_number` separate library preps or is one a re-sequenced technical replicate? That is, might there be double counting of UMIs across `plate_number`? Daniel tells me they are separate library preps, i.e. no double counting.
...
2. The majority of log-fold changes between batches are small at both the sample and plate levels.
    a. At the sample level, `r percent(sum(abs(d) < 0.5) / length(d))` UMI counts have $|logFC| < 0.5$ between batches and only $5\%$ of UMI counts have $|logFC| >$ `r round(quantile(abs(d), 0.95), 2)`.
    b. At the plate level, `r percent(sum(abs(D) < 0.5) / length(D))` UMI counts have $|logFC| < 0.5$ between batches and only $5\%$ of genes have $|logFC| >$ `r round(quantile(abs(D), 0.95), 2)`.
3. The hemoglobin subunit genes with lower average expression appear more highly expressed in the `LC476` plate, whereas those hemoglobin subunit genes with higher average expression appear to not be differentially expressed.
